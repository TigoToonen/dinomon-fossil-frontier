...
- **RONDE-8-NERFS TERUGGEDRAAID (Tigo's keuze, na de move-fix)**: de move-fix onthulde dat de gym-audit van ronde 8 op vervuilde data rustte (testteams konden niet aanvallen). Hermeting mét werkende movesets: de ORIGINELE (on-generfde) gyms zijn goed gebalanceerd voor een voorbereide speler — Volt 98% / Marina 68% / Valdez 65%. De nerfs waren dus onnodig en maakten Valdez te makkelijk (65→85%). Volt/Marina/Valdez exact hersteld naar Tigo's origineel (trainers.js v102, diff vs pre-nerf = leeg). LES: valideer een balans-conclusie nooit op auto-gegenereerde teams zonder eerst de moveset-integriteit te checken.
